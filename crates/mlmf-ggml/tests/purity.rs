//! C3: `mlmf-ggml` performs no I/O. Enforced at the source level, not by
//! convention — and **structurally**, not by substring match.
//!
//! Copied from `mlmf-core`'s gate of the same name (see that file's header
//! for the full history of why a substring match is not safe). The first
//! version of that gate compared raw source text against the literal
//! needles `"std::fs"` and `"std::net"`. That is defeated by a grouped
//! import: `use std::{fs, path::Path};` contains neither needle and is the
//! form `rustfmt` itself emits under `imports_granularity = "Crate"`. A
//! module doing real `fs::read`, a real `TcpStream::connect` and a real
//! `Command::new("curl")` compiled and exported from that crate with the
//! whole suite green.
//!
...
/// Crates that *are* I/O or networking. Naming one anywhere in `mlmf-ggml`
...
/// The `std` submodules `mlmf-ggml` may name.
///
/// Deliberately an allow-list, narrower than `mlmf-core`'s: this crate's own
/// code writes no `use std::` at all (only prelude items), so the list is
/// kept tight to the pure geometry/arithmetic vocabulary it could plausibly
/// need — `std::fs`, `std::net`, `std::process`, `std::env`, `std::os`,
/// `std::io`, `std::thread`, `std::time`, `std::path` and everything else
/// `mlmf-core` allows for its own reasons are all absent here.
...
fn ggml_performs_no_io() {
    ...
    assert!(
        violations.is_empty(),
        "mlmf-ggml must perform no I/O (C3); found:\n  {}",
        violations.join("\n  ")
    );
}
...
fn the_gate_does_not_cry_wolf() {
    ...
    //
    // Unlike `mlmf-core`'s copy of this test, the accepted `std::` imports
    // here are drawn only from `mlmf-ggml`'s narrower ALLOWED_STD — a form
    // that is legitimate for `mlmf-core` (`std::path`, `std::collections`,
    // `std::borrow`, `std::ops`, ...) is *correctly* rejected here, because
    // this crate's allow-list does not carry those entries.
