//! C2 (direct half) and the manifest half of C3/C5: `mlmf-ggml`'s direct
//! dependencies are pinned to an allow-list, and no table other than a plain
//! `[dependencies]` may introduce one.
//!
//! Copied from `mlmf-core`'s gate of the same name. That gate's first
//! version matched section headers with `line == "[dependencies]"`, which
//! silently switched collection **off** for every other header. That let
//! three standard, non-adversarial Cargo forms add a dependency with the
//! suite green:
//!
...
//! being skipped.
//!
//! Unlike `mlmf-core`, `mlmf-ggml` gets no transitive-dependency snapshot:
//! it adds zero third-party dependencies, so its transitive set is exactly
//! `mlmf-core`'s, which `mlmf-core` already gates (`tests/transitive_deps.rs`
//! there). A second snapshot here would be a duplicate that must be updated
//! in lockstep, and a stale duplicate is worse than none.
...
fn direct_dependencies_match_allowlist() {
    ...
    assert_eq!(
        facts.deps,
        allow_list(),
        "mlmf-ggml's direct dependencies changed.\n\
         If this is intended, update crates/mlmf-ggml/tests/direct-deps.allow. \
         mlmf-ggml carries no transitive snapshot of its own (it adds zero \
         third-party dependencies), so also check whether mlmf-core's own \
         transitive snapshot needs re-blessing \
         (crates/mlmf-core: scripts/check-deps.sh --bless)."
    );
}
...
fn no_table_other_than_plain_dependencies_may_declare_an_edge() {
    ...
    assert!(
        ...
        "mlmf-ggml's manifest declares dependencies through a table that is \
         not `[dependencies]`: {:?}.\n\
...
fn the_manifest_names_no_io_crate_anywhere() {
    ...
    assert!(
        found.is_empty(),
        "mlmf-ggml's manifest names an I/O or codegen crate (C3/C5):\n  {}",
        found.join("\n  ")
    );
}
