...
4. Replace `Entry::unreadable` with a process-wide `static UNREADABLE: OnceLock<Unrecognized>` initialised on first use — the shape a first draft of this plan actually specified. Every test **within** a file stays green, and `two_files_each_report_their_own_unreadable_key` goes red showing `b.gguf`'s key reported as `alpha`. Restore. This is the sabotage worth doing slowly: the defect is invisible to any single-file test, which is why the test parses two.
