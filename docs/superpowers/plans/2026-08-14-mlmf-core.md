...
**1. Spec coverage.** This plan implements the parts of the spec that live in `mlmf-core`: §4.1 (Encoding, Task 4), §4.2 (Shape and TensorDescriptor, Tasks 3 and 5), §4.3 (MetaValue, Task 6), §4.4 (Error, Task 2), §4.5 (traits, Task 8), §4.6 (alignment, Task 5), **§7's two-tier unknown *classification*** (fatal in Task 2, loud in Task 7), and constraints C1/C2/C3/C5/C7 (Task 1).

> **Correction (2026-08-14, post-implementation review).** The line above originally read "§7's two-tier unknown handling", which claimed more than this plan delivers. **§7 has four obligations, not two.** Beyond the fatal/loud split it states **U-1 (agreement)** — "CI asserts the report is empty across a corpus of known-good checkpoints" — and **U-2 (currency)** — "Corpus currency is tracked and can fail independently of corpus agreement." Neither appeared in either list below, so a clause the spec numbered for other projects to cite was quietly discharged. Neither is implementable inside `mlmf-core`, which has no I/O and no format parser, which is exactly why it must be **recorded as owed** rather than absorbed into a "§7 covered" claim. The repository is currently one step worse than the state §7's own justification warns against — zero of two, not one of two: there is no corpus directory, no currency record, and no CI to run either. See §9 clause 5.3: "Agreement and currency are separate obligations, each able to fail alone."

Deliberately **not** in this plan, because they belong to later crates: §3.2 (HF layout split), §5 (canonical key vocabulary — `mlmf-meta`), §6 (declared/absent/supplied — write paths), §7 **U-1 and U-2** (corpus agreement and corpus currency — the `mlmf-gguf` plan, which per §12 step 2 is the first that can parse a corpus; it needs a corpus directory where each entry records its source URL and fetch date, a test asserting `Report::is_empty()` across it for U-1, and a **separate** test that fails on staleness against that recorded date or upstream revision for U-2 — separate so it can fail alone, per PV-1 and clause 5.3), §8 (conversion and measured error), §9 §1/§4 (chat templates — `mlmf-meta`), §9 §7 AD-1/AD-2/AD-3 (adoption gates — the `mlmf-gguf` and `mlmf-pickle` plans), §9 §6 HUB-1/HUB-2 (`mlmf-source-hub`), §11 (disposition of existing code). Each gets its own plan per the §12 sequencing.

**2. Placeholder scan.** No TBD, TODO, "handle edge cases", or "similar to Task N". Every code step contains complete compilable code.

**3. Type consistency.** `Result`/`Error`/`ErrorKind` (Task 2) are used unchanged in Tasks 4, 5 and 8. `DType::size`/`alignment` (Task 3) are called by `Encoding` (Task 4). `Shape::elem_count` returns `Result<u64>` (Task 3, made fallible in post-implementation review — the unchecked `product()` wrapped silently in release, so a tensor declaring 2^64 elements validated as a zero-byte tensor) and `Encoding::byte_size` takes `u64` (Task 4) and `TensorDescriptor::validate` propagates it with `?` (Task 5) — consistent. `BlockSpec::elements_per_block`/`bytes_per_block` are `u64` throughout. `MetaValue` (Task 6) is used by `UnrecognizedKind` (Task 7) and `MetadataSource` (Task 8). `MetaValue::as_str` returns `Option<&String>`, and the Task 8 test compares against `Some(&"llama".to_string())` — consistent.
