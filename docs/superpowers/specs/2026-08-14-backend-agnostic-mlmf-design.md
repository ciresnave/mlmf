...
- **AD-1.** Before Fuel switches its dependency, a **differential harness** parses a real corpus with both the origin implementation and the ported one and asserts **byte-identical** tensor name, ggml type code, and absolute byte range, and byte-identical tensor bytes. Not "equivalent" — identical. ⚠️ **Shape is the one exception, measured rather than assumed:** fuel's GGUF reader reverses the file's declared dimension order (`fuel-formats/src/gguf.rs`, `dimensions.reverse()`); `mlmf_core::Shape` never reorders. The two descriptors' `shape` fields are therefore equal only after an explicit un-reversal, not byte-for-byte — AD-1's harness asserts the un-reversed values agree, which is the same tensor under each side's own declared-order convention, not literal identity of the two structs. Built and run 2026-09-24 (`mlmf-conformance/tests/fuel_differential.rs`): 25/28 corpus files agree on every other field; 3/28 show the AD-1-anticipated divergence below.
