...
- **AD-1.** Before Fuel switches its dependency, a **differential harness** parses a real corpus with both the origin implementation and the ported one and asserts **byte-identical** tensor descriptors *and* byte-identical tensor bytes. Not "equivalent" — identical. ⚠️ **This requirement is NOT yet fully met, and the GGUF/GGML run below is partial evidence toward it, not a closed gate:**
  - **Built and run for GGUF/GGML, 2026-09-24** (`mlmf-conformance/tests/fuel_differential.rs`, `--features fuel-differential`), against 28 corpus files: tensor **name**, **ggml type code**, and **absolute byte range** agree on 25/28; the other 3 show the AD-1-anticipated ggml-coverage divergence (§9 §7 accepted losses). **Metadata key sets agree**, both directions (values are not compared). **Tensor bytes are NOT read or compared here** — only their computed ranges. So AD-1's payload-identity half is **untested**, not satisfied; a range agreeing is necessary evidence, not the thing AD-1 actually asks for.
  - **Shape is a separate, measured exception**, not part of the above gap: fuel's GGUF reader reverses the file's declared dimension order (`fuel-formats/src/gguf.rs`, `dimensions.reverse()`); `mlmf_core::Shape` never reorders. The two descriptors' `shape` fields are equal only after an explicit un-reversal — the harness asserts the un-reversed values agree, which is the same tensor under each side's own declared-order convention, and is a real, permanent asymmetry in the two formats' conventions rather than a gap to close.
  - **Built and run for safetensors, 2026-09-25** (`mlmf-conformance/tests/fuel_differential_safetensors.rs`, same `--features fuel-differential`), against a nine-file corpus (two real single-file downloads, one five-shard checkpoint, two more single-file tiny models across three architectures): tensor **name**, **dtype**, **shape**, and **absolute byte range** agree on all nine, and `__metadata__` agrees **key-for-key including values** (safetensors' metadata is flat `string -> string` on both sides, unlike GGUF's typed KV block, so a full value comparison costs nothing extra). **Zero divergences** — unlike the GGUF/GGML run above, widening this corpus did not surface one. **Also runs in CI now**, against a seven-file pinned-by-revision-SHA subset (`.github/workflows/ci.yml`), proven able to fail there by a deliberate sabotage-and-revert (one file's download removed, both OS runners went red naming the incomplete corpus, then reverted to green). **Tensor bytes are NOT read or compared here either** — same gap as GGUF, restated rather than let a clean result imply a stronger claim: AD-1's payload-identity half remains untested for safetensors exactly as it does for GGUF.
- **AD-2.** The differential must be **proven able to fail**: deliberately sabotage the port (perturb an offset, drop a metadata key, flip a dim order) and confirm the harness catches each. An unfalsified differential is PV-1's failure wearing a test's clothes. **Run on GGUF, 2026-09-24**: a `+1` sabotage on a computed byte offset was caught, naming the file and tensor, then reverted. **Run on safetensors, 2026-09-25**, twice: an offset sabotage (mechanism check) and a dtype-mapping sabotage targeted at the newly-added sharded file specifically (proving the *new* corpus rows are actually asserted, not merely counted) — both caught, both reverted. **Run again in CI, 2026-09-25**: one corpus file's download deliberately removed, both OS runners went red naming the incomplete corpus, then reverted to green — the first time this differential has been proven able to fail in CI rather than only locally.
...
**Accepted losses, ruled by CireSnave 2026-09-25**, quoted verbatim: *"I already said we're transitioning from Fuel's fuel-formats to MLMF. I'm fine with the losses you mentioned that will happen during that transition."* The AD-1 run above (§9 §7) is what turned the migration from a stated direction into a trade with measured costs in front of him when he ruled. Five gaps, accepted as of this date and NOT waived — each stays open as a scheduling item, not a closed question, until someone measures the cost of closing it and it is either built or explicitly re-deferred:

- **GGUF v1.** `fuel_formats::gguf::VersionedMagic::GgufV1` is a real, handled case; `mlmf-gguf` refuses it by name (`GgufError::UnsupportedVersion`). Cost already measured (`crates/mlmf-gguf/src/lib.rs`'s own "GGUF v1 is refused, and that is a decision" section): v1 uses `u32` where v2+ uses `u64`, for the tensor/KV counts and every length-prefixed string and array — a second parse path at every read site, not a constant change.
- **`impl Read`/streaming over an un-materialized source.** `fuel-formats` takes `impl Read`/`impl Seek`/`&[u8]`/`Cow<[u8]>`; `mlmf-core`'s `RangedSource` takes a caller-supplied buffer and never names `std::io`, and `mlmf-source-file` always materializes the whole file (mmap or full read) before anything reads from it. A new source crate is needed, not a design change to `mlmf-core` — confirmed, not merely asserted: `RangedSource::read_range` takes `&self`, so a naive `Seek`-based implementation over a shared reference reproduces the shared-mutable-cursor hazard `mlmf-source-file`'s own doc already rejected once. Closing this needs either interior mutability with locking, or a trait-level change to `&mut self` that every existing implementor absorbs.
- **`mlmf-pickle`.** No pickle reader exists in the `mlmf-*` crates. Fuel's own VM (`fuel-formats/src/pickle.rs`) is 754 lines and is the port target this spec (§11, §12 step 6) already names; AD-3 above is its adoption gate.
- **GGML-legacy (GGJT) container.** `mlmf-ggml` is type-geometry only (block sizes, alignment) — it does not parse a container. Fuel's reader (`fuel-formats/src/ggml.rs`) is 203 lines. Not yet scoped whether it is worth building versus Fuel keeping it permanently (OQ-3 below is the same question for `imatrix`, unilaterally noted rather than agreed — see that entry's own provenance gap).
- **`mlmf-hf-layout`'s metadata half.** Only the shard-index half (`Task 4`, "Fuel's half") is built. A metadata-half plan exists and was superseded before implementation for cause — two audit rounds found twelve blocking findings, four of the second round created by the first round's own fixes (`docs/superpowers/plans/2026-09-05-mlmf-hf-layout-metadata-DEFERRED.md`) — so a rewrite starts fresh, informed by that record, not from zero.
...
- **Superseded** — a merged crate does this now, so the file is redundant. ⚠️ **This is a statement about the REPLACEMENT, not about the WORK.** It does not mean the file can be deleted: measured 2026-09-06, the three Superseded rows had **0**, **1** and **261** call sites respectively. **A Superseded row must carry its measured caller count**, or "redundant" reads as "delete me" for a file with 261 callers exactly as loudly as for one with none.
...
| `config.rs` | 588 | Planned | → `mlmf-hf-layout` + `mlmf-meta` as **raw + typed accessors**, not a normalized struct (§10). ⚠️ **MEASURED INSTANCE OF WHY, 2026-09-06:** `head_dim()` and `kv_head_dim()` are hardcoded as `hidden_size / num_attention_heads`, and the struct has **no field able to hold a declared head dimension** — so an explicitly-declared one cannot be represented at all. In the corpus, `ggml-vocab-gemma-4.gguf` declares `gemma4.attention.key_length = 512` while `embedding_length / head_count` = **176**: the division is **2.909x wrong** and silently so. `validate()` also rejects any config where `hidden_size % num_attention_heads != 0`, which is legal when the head dimension is declared. **Both are interpretation, which §10 places outside MLMF** — the raw-accessor ruling prevents this by construction, and this row records the concrete defect so the port is checkable rather than merely principled. ⚠️ **AND "READ THE DECLARED KEY INSTEAD OF DIVIDING" IS INSUFFICIENT — the obvious remedy does not work.** `ggml-vocab-gemma-4.gguf` declares **two** attention geometries: `key_length` 512 **and** `key_length_swa` 256, likewise `value_length`/`value_length_swa` and `rope.dimension_count`/`rope.dimension_count_swa`, selected per layer by a 30-element `attention.sliding_window_pattern`. A reader holding ONE head dimension cannot represent it **however it derives the value**. ⚠️ **Decisive for §10: `attention.head_count_kv` in that file is a 30-ELEMENT PER-LAYER ARRAY** `[8,8,8,8,8,2,…]`, while `config.rs` holds `pub num_key_value_heads: usize` — a scalar. **A normalized struct cannot hold this checkpoint at all, so the raw-accessor ruling is not a preference here; it is the only shape that can represent the file.** ⚠️ **AND A SECOND, SMALLER INSTANCE OF THE SAME STRUCT PROBLEM, filed 2026-09-06:** `ModelConfig.activation_function` is returned as `"silu"` for **every architecture** by `src/formats/gguf.rs::config_from_gguf`. GGUF declares no activation for any of the **fourteen** distinct architectures in the corpus, so it cannot be read — **but the field is not optional, so a constant is asserted with nothing behind it.** The remedy is not a better constant and not an architecture-to-activation table (that would ADD the interpretation §10 removes); it is that the struct demands a field the format cannot supply. Recorded here rather than fixed. *(Found by lightbulb from mlmf's corpus census; confirmed here by an independent reader. Scope: lightbulb has NINE independent `hidden_size / num_heads` sites including one on their serving path — this row's single accessor must not be used to size that work. Not measured: whether key and value head dimensions ever differ — 1 corpus file declares them and they are equal, and it is the same file in both corpora, so agreement between us is not corroboration.)* |
...
| `formats/gguf.rs` | 317 | **Superseded, 1 caller** | **by `mlmf-gguf`** — the replacement this row used to ask for now exists and is candlelight-free. What it replaces is a 317-line shim over `candlelight::quantized::gguf_file` that still imports `candlelight::VarBuilder`. ⚠️ **NOT deletable as-is: `universal_loader.rs:186` does `use crate::formats::gguf::load_gguf`.** Re-pointing it at `mlmf-gguf` needs a root→`mlmf-*` dependency **that does not exist today** |
...
| `error.rs` | 161 | **Superseded, 261 call sites** | **by `mlmf-core::error`** (§4.4). What it replaces is an enum whose variants wrap `candlelight::Error` and `safetensors::SafeTensorError` — dependency edges the split exists to remove. ⚠️ **NOT a deletion: 261 `Error::` call sites across 26 files in `src/`, and the root crate does not depend on `mlmf-core` at all** (it appears in `Cargo.toml` only as a comment and a workspace member). ⚠️ **AND IT IS NOT A MIGRATION AT ALL — THE NAMED DESTINATION CANNOT RECEIVE IT.** Measured 2026-09-06: root has **15** variants, `mlmf-core::ErrorKind` has **14**, and the **name overlap is ZERO**. Core's taxonomy is FORMAT PARSING (`Truncated`, `RaggedRow`, `Misaligned`, `UnknownTypeCode`); the root's is LOADING POLICY (`ModelLoading`, `InvalidConfig`, `DeviceValidation`, `TensorNameMapping`). **That is the §3.1 two-axis split working as designed, not a naming drift** — and of the 210 constructor calls, **186 are the four kinds core has no target for** (`model_loading` 113, `invalid_config` 27, `io_error` 26, `invalid_format` 20). ⚠️ **`DeviceValidation`/`CudaValidation` have no home by a ruling ALREADY RECORDED ON ANOTHER ROW** — `validation.rs` says *"`mlmf-core` has no device type, so that half has no home by construction"* — **so the two rows contradict each other and neither knows it.** Core's `Source(Box<dyn Error>)` makes wrapping mechanically possible, but routing 210 typed constructors through an opaque box **destroys the taxonomy on a published crate**: a deletion wearing a migration's name. **OPEN QUESTION, not work:** does the umbrella keep a loading-policy error type with `mlmf_core::Error` as a *source*, or does core grow policy variants against §3.1? ⚠️ **AND THE SEMANTIC OVERLAP IS NOT ZERO JUST BECAUSE THE NAMES ARE — measured separately, because assuming it would have been the same error one level down.** Classified by MEANING, exactly **one** of the 15 is arguably format-axis: `InvalidFormat`. The other fourteen are third-party wrappers (`Io` — source axis by §3.1, `Json`, `Candle`, `SafeTensors`, `Regex`, `Anyhow`), INTERPRETATION the charter places with Fuel (`UnknownArchitecture` detects architecture from tensor names; `TensorNameMapping`), device concerns already ruled homeless, or loading policy. ⚠️ **And the one candidate still cannot move, for a reason unrelated to naming: core's `Malformed` carries `format` and a byte `offset`; root's `InvalidFormat` carries `message: String` and nothing else.** Six root variants are message-only where core has 26 structured fields across 14 variants — **the taxonomies differ in PAYLOAD RICHNESS, not just vocabulary, so even the movable-in-principle variant would require INVENTING a byte offset at 20 call sites that never recorded one.** Until this is ruled, the row states a destination that cannot accept the thing |
...
| `formats/safetensors.rs` | 111 | **DELETED** | **by `mlmf-safetensors`** — the reader landed in #6. **Deleted 2026-09-06; 0 callers, verified with a control.** ⚠️ **This row previously said "This file's `load_mmaped_safetensors` does not memory-map" — TRUE, and the WRONG FUNCTION.** That one had **zero callers**. The live path is **`loader.rs::load_safetensors_mmap`**, which `use_mmap` actually reaches, carries the identical forwarding, and **is NOT in this table**. Deleting this file did **not** fix mmap. See the note below the table |
...
⚠️ **"589 lines" IS NOT 589 LINES OF DELETION, and this sentence used to imply it was.** Measured 2026-09-06: **111 deletable** (`formats/safetensors.rs`, 0 callers — done), **317 with one live caller**, **161 with 261 call sites**. The last two are blocked behind a root→`mlmf-*` dependency edge that does not exist, which is the actual "retire the legacy crate" work rather than housekeeping.
...
| Fuel migrates to the `mlmf-*` crates, never the root `mlmf` package (§9 §7 accepted losses) | CireSnave, 2026-09-25, quoted verbatim: *"I already said we're transitioning from Fuel's fuel-formats to MLMF. I'm fine with the losses you mentioned that will happen during that transition."* |
| Accepting, not waiving, five measured gaps at migration time: GGUF v1, `impl Read`/streaming, `mlmf-pickle`, GGML-legacy container, `mlmf-hf-layout`'s metadata half | CireSnave, 2026-09-25 (same quote; costs measured in §9 §7 above, each a scheduling item, not closed) |
