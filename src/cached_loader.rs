...
impl CachedModelLoader {
    ...
    pub async fn load_safetensors<P: AsRef<Path>>(
        &self,
        model_path: P,
        options: LoadOptions,
    ) -> Result<Arc<LoadedModel>> {
        ...
        let cache_key = format!(
            "safetensors_{}",
            self.create_cache_key(&model_path, &options)
        );
    ...
    async fn load_model_direct(
        ...
        if let Some(ext) = model_path.extension() {
            match ext.to_str() {
                Some("safetensors") => crate::loader::load_safetensors(model_path, options),
                #[cfg(feature = "gguf")]
                Some("gguf") => crate::formats::load_gguf(model_path, &options),
                _ => {
...
pub async fn load_cached<P: AsRef<Path>>(
    model_path: P,
    options: LoadOptions,
) -> Result<Arc<LoadedModel>> {
...
pub async fn load_safetensors_cached<P: AsRef<Path>>(
    model_path: P,
    options: LoadOptions,
) -> Result<Arc<LoadedModel>> {
    global_cached_loader()
        .load_safetensors(model_path, options)
        .await
}
